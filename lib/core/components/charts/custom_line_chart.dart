...
class _CustomLineChartState extends BaseState<CustomLineChart> {
  ...
  Widget build(BuildContext context) {
    return LineChart(
        LineChartData(
            ...
            lineBarsData: [
              LineChartBarData(
                  spots: List.generate(widget.lineChartValue?.length ?? 0, (index) => FlSpot(index.toDouble(), widget.lineChartValue?[index] ?? 0)),
                  show: true,
  ...
  Widget leftTitleWidgets(double value, TitleMeta meta) {
    ...
    return Text((value ~/ 1000).toStringIfNull ?? "", style: style, textAlign: TextAlign.left);
  }
}
