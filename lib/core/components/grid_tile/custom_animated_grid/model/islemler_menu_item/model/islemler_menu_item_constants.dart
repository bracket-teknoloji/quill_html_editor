...
class IslemlerMenuItemConstants<T> {
  ...
  List<GridItemModel?> islemlerList = <GridItemModel?>[];
  T? model;
  // T? get model2 => model;
  ...
  IslemlerMenuItemConstants({required this.islemtipi, List<GridItemModel?>? raporlar, this.model, this.siparisTipi}) {
    if (islemtipi == IslemTipiEnum.stok) {
      ...
      islemlerList.addAll(raporlar ?? <GridItemModel?>[]);
    }
  }
  ...
  GridItemModel? get kopyala => GridItemModel.islemler(
      title: "Kopyala", iconData: Icons.copy_outlined, onTap: () async => await Get.toNamed(islemtipi.route, arguments: BaseEditModel(model: model, baseEditEnum: BaseEditEnum.kopyala)));
  //* Siparis
  ...
  Future<bool> kilitRequest(BaseSiparisEditModel siparisModel, int yeniTipi) async {
    final GenericResponseModel<NetworkManagerMixin> result = await _networkManager.dioPost<SiparisEditRequestModel>(
            ...
            body: Column(children: <Widget>[
              ...
              Form(
                ...
                child: CustomTextField(
                    ...
                    suffix: IconButton(
                        onPressed: () async {
                          final GenericResponseModel<NetworkManagerMixin> result = await _networkManager.dioGet<BaseSiparisEditModel>(
                      ...
                      child: ElevatedButton(
                          onPressed: () async {
                            if (formKey.currentState?.validate() ?? false) {
                              final GenericResponseModel<NetworkManagerMixin> result = await NetworkManager().dioPost<SiparisEditRequestModel>(
            ...
            body: Column(
              ...
              children: <Widget>[
                ...
                CustomTextField(
                    ...
                    controller: controller,
                    isMust: true,
                    onChanged: (String p0) => kodDegistirModel.hedefCari = p0,
                    suffix: Wrap(
                      children: <Widget>[
                        IconButton(
                          onPressed: () async {
                              ...
                              if (controller.text != "") {
                                DialogManager().showAreYouSureDialog(() async {
                                  final GenericResponseModel<NetworkManagerMixin> result =
                                      await NetworkManager().dioPost<KodDegistirModel>(path: ApiUrls.kodDegistir, bodyModel: KodDegistirModel(), data: kodDegistirModel.toJson());
                                  if (result.success == true) {
                                    Get.back();
                                    DialogManager().showSuccessSnackBar("Başarılı");
                                  }
  ...
  GridItemModel? get krediKartiTahsilati =>
      GridItemModel.islemler(title: "Kredi Kartı Tahsilatı", iconData: Icons.list_alt_rounded, onTap: () async => await Get.toNamed("/mainPage/krediKartiTahsilati"));
  GridItemModel? get nakitTahsilat => GridItemModel.islemler(title: "Nakit Tahsilat", iconData: Icons.list_alt_rounded, onTap: () async => await Get.toNamed("/mainPage/nakitTahsilat"));
  ...
  GridItemModel? get nakitOdeme => GridItemModel.islemler(title: "Nakit Ödeme", iconData: Icons.list_alt_rounded, onTap: () async => await Get.toNamed("/mainPage/nakitOdeme"));
