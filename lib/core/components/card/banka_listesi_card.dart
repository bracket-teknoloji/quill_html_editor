...
class _KasaListesiCardState extends BaseState<KasaListesiCard> {
  ...
  Widget build(BuildContext context) {
    return Card(
      child: ListTile(
          onTap: () async {
            await dialogManager.showKasaGridViewDialog(widget.item,
                onSelected: (p0) => widget.onSelected?.call(p0));
          },
          leading: CircleAvatar(
            child: Text(widget.item?.kasaTanimi?[0] ?? ""),
          ),
          title: Text(
              "${widget.item?.kasaKodu != null ? '${widget.item?.kasaKodu} - ' : ''}${widget.item?.kasaTanimi ?? ""}"),
          subtitle: Text(
          ...
          trailing: Text.rich(
            TextSpan(
              ...
              TextSpan(
                  style: TextStyle(
                      color: ((widget.item?.dovizli == "E"
                                  ? (widget.item?.devirliDovizBakiye ??
                                      widget.item?.dovizDevirTutari ??
                                      0)
