...
final class EFaturaListesiCard extends StatefulWidget {
  const EFaturaListesiCard({
    required this.eBelgeListesiModel,
    required this.eBelgeEnum,
...
final class _EFaturaListesiCardState extends BaseState<EFaturaListesiCard> {
      ...
      onTap: () async {
        await bottomSheetDialogManager.showBottomSheetDialog(
          ...
          children: [
            ...
            if (((!model.gelenMi && model.eArsivMi) ||
                    (model.cariKayitliMi && model.eFaturaMi) ||
                    (!model.gelenMi && model.eFaturaMi)) &&
                model.cariKayitliMi)
              cariIslemleri,
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              final efaturaList = [
                ...
                Text(
                  "Onay: ${model.onayAciklama ?? ""}",
                  style: TextStyle(color: model.yanitBekliyorMu ? UIHelper.primaryColor : null),
                ),
                if (!model.gelenMi)
                  InkWell(
                    onTap: showCevapAciklamaSnackBar,
                ...
                if (model.dovizliMi)
                  Text(
                    "Genel Döv. Toplam: ${model.dovizTutari.commaSeparatedWithDecimalDigits(OndalikEnum.dovizTutari)} ${model.dovizAdi}",
                  ),
              ];
              final eArsivList = [
                Text("Kayıt Tarihi: ${model.kayittarihi.toDateString}"),
    ...
    onTap: () async {
      ...
      await bottomSheetDialogManager.showBottomSheetDialog(
        ...
        body: Column(
          ...
          children: [
            if (model.kontrolAciklama != null)
              Text(
                "* Kontrolü kaldırmak için açıklamayı boş bırakın.",
                style: theme.textTheme.bodyLarge?.copyWith(color: UIHelper.primaryColor),
              ),
            CustomTextField(
              labelText: "Kontrol Açıklaması",
              controller: controller,
              suffix: IconButton(onPressed: controller.clear, icon: const Icon(Icons.close_outlined)),
            ),
            ElevatedButton(
              onPressed: () async {
                if (controller.text == "") {
    ...
    onTap: () async {
      ...
      await bottomSheetDialogManager.showBottomSheetDialog(
        ...
        body: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            CustomTextField(labelText: "Açıklama", controller: controller),
            Row(
              children: [
                Expanded(
                  flex: 20,
        ...
        body: Column(
          ...
          children: [
            CustomTextField(
              ...
              suffixMore: true,
              onTap: () async {
                final List<YaziciList?> yaziciListe =
                    CacheManager.getAnaVeri?.paramModel?.yaziciList ?? <YaziciList?>[];
  ...
  BottomSheetModel get alisFaturasiOlustur => BottomSheetModel(
    ...
    onTap: () async {
      ...
      final cariModel = await networkManager.getCariModel(
        CariRequestModel(
          kod: [""],
          vergiNo: widget.eBelgeListesiModel.vergiNo,
          eFaturaGoster: true,
          plasiyerKisitiYok: true,
        ),
      );
  ...
  BottomSheetModel get dekontOlustur => BottomSheetModel(
    ...
    onTap: () async {
      Get.back();
      final result = await Get.toNamed("/mainPage/dekontEBelgedenEkle", arguments: model);
      if (result != null) {
        widget.onRefresh.call(true);
      }
    },
  ...
  BottomSheetModel get faturaGoruntule => BottomSheetModel(
    title:
        "${EditTipiEnum.values.firstWhereOrNull((element) => element.rawValue == model.belgeTuru)?.getName} Görüntüle",
    iconWidget: Icons.preview_outlined,
    onTap: () async {
      Get.back();
      await Get.toNamed(
        ...
        arguments: BaseEditModel<SiparisEditRequestModel>(
          ...
          editTipiEnum: EditTipiEnum.values.firstWhereOrNull(
            (element) => element.rawValue == widget.eBelgeListesiModel.belgeTuru,
          ),
        ),
      );
    },
  ...
  void showCevapAciklamaSnackBar() => dialogManager.showInfoSnackBar(
    "Cevap Kodu : ${model.cevapKodu.toStringIfNotNull ?? "0"}\n${model.cevapAciklama ?? ""}",
  );
}
