...
import 'base_stok_edit_genel/view/base_stok_edit_fiyat_listesi.dart';
...
class _BaseStokEditingViewState extends BaseState<BaseStokEditingView> {
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> viewList = [
      BaseStokEditGenelView(model: widget.model),
      const Center(
        child: Text(""),
      ),
      const Center(
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [Visibility(visible: widget.model?.baseEditEnum == BaseEditEnum.duzenle, child: IconButton(onPressed: () {}, icon: const Icon(Icons.save_outlined)))],
          bottom: TabBar(
            tabs: tabList,
          ),
        ),
