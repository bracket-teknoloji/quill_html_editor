...
import "../../../../init/platform_implementations.dart"
    if (dart.library.html) "package:picker/core/init/web/file_downloader.dart"
    show fileDownload;
...
final class _PDFViewerViewState extends BaseState<PDFViewerView> {
  ...
  AppBar appBar(BuildContext context) => AppBar(
    title: AppBarTitle(title: widget.title, subtitle: widget.pdfData!.dicParams?.belgeNo ?? ""),
    actions: [
      IconButton(
        onPressed: () async {
          await fileChecker();
          ...
          onPressed: () async {
            if (kIsWeb) {
              return fileDownload(
                ...
                "${widget.pdfData?.raporOzelKod}_${widget.pdfData?.dicParams?.belgeNo}.pdf",
              );
            }
  ...
  Future getData() async {
    ...
    final result = await networkManager.getPDF(
      (widget.serbestMi == true
              ? widget.pdfData?.copyWith(dicParamsMap: StaticVariables.instance.serbestDicParams)
              : widget.pdfData) ??
          PdfModel(),
    );
    if (result.isSuccess) {
    ...
    if (viewModel.pdfModel != null) {
      ...
      if (kIsWeb) {
        file = XFile.fromData(
          ...
          name: viewModel.pdfModel?.dosyaAdi ?? "file",
        );
      }
