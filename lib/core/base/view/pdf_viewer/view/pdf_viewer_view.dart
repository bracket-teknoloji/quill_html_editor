...
class _PDFViewerViewState extends BaseState<PDFViewerView> {
        ...
        bottom: PreferredSize(
          ...
          child: AppBarPreferedSizedBottom(
            children: <AppBarButton?>[
              if (widget.filterBottomSheet != null) AppBarButton(
                      ...
                      child: const Text("Filtrele")) else null,
              AppBarButton(
        ...
        if (viewModel.futureController.value == true && viewModel.pdfFile != null) {
          return Observer(
              builder: (_) => SfPdfViewer.file(
                    ...
                    onDocumentLoaded: (PdfDocumentLoadedDetails details) => viewModel.changePageCounter(details.document.pages.count),
                    onPageChanged: (PdfPageChangedDetails details) => viewModel.changeCurrentPage(details.newPageNumber - 1),
                  ));
  ...
  Future getData() async {
    ...
    final GenericResponseModel<NetworkManagerMixin> result = await networkManager.getPDF(widget.pdfData ?? PdfModel());
    if (result.data != null) {
  ...
  Future<void> fileChecker() async {
    if (await getFile != null) {
      await Share.shareXFiles(<XFile>[XFile((await getFile)!.path)], subject: "Pdf Paylaşımı");
    } else {
  ...
  Future<File?> get getFile async {
    ...
    final RandomAccessFile fileWriter = file.openSync(mode: FileMode.write);
