...
class _KalemEkleViewState extends BaseState<KalemEkleView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: appBar(), floatingActionButton: fab(), body: body(context));
  }
  ...
  AppBar appBar() {
    return AppBar(
      title: Observer(builder: (_) {
        return AppBarTitle(
            title: "Kalem Ekle", subtitle: viewModel.model?.stokAdi ?? "");
      }),
      actions: [
        IconButton(
            onPressed: () async {
        ...
        IconButton(
            onPressed: () {
              if (formKey.currentState?.validate() ?? false) {
                ...
                viewModel.kalemModel.kalemList
                    ?.map((e) => e.miktar = viewModel.kalemModel.miktar2)
                    .toList();
                BaseSiparisEditModel.instance.kalemList ??= [];
                if (BaseSiparisEditModel.instance.kalemList?.any((element) =>
                        element.stokKodu == viewModel.kalemModel.stokKodu) ??
                    false) {
                  //replace
                  BaseSiparisEditModel.instance.kalemList?[BaseSiparisEditModel
                          .instance.kalemList
                          ?.indexWhere((element) =>
                              element.stokKodu ==
                              viewModel.kalemModel.stokKodu) ??
                      0] = viewModel.kalemModel
                    ..paketMi = widget.stokListesiModel?.paketMi;
                } else {
                  BaseSiparisEditModel.instance.kalemList?.add(
                ...
                dialogManager
                    .showErrorSnackBar("Lütfen gerekli alanları doldurunuz");
              }
            },
                ...
                child: Column(
                  children: [
                        ...
                        children: [
                          Expanded(
                              child: CustomTextField(
                                  ...
                                  onChanged: (p0) =>
                                      viewModel.kalemModel.stokAdi = p0)),
                          Expanded(
                              child: CustomTextField(
                                  labelText: "Muh. Kodu",
                                  suffixMore: true,
                                  ...
                                  onTap: () async {
                                    var result = await bottomSheetDialogManager
                                        .showMuhasebeKoduBottomSheetDialog(
                                            context);
                                    if (result != null) {
                                      muhKoduController.text = result.adi ?? "";
                                      viewModel.setMuhasebeKodu((StaticVariables
                                                  .instance.isMusteriSiparisleri
                                              ? result.satisHesabi
                                              : result.satisHesabi) ??
                                          "");
                                    }
                                  })).yetkiVarMi((viewModel.kalemModel.stokKodu
                                      ?.startsWith("HIZ") ??
                                  false) &&
                              yetkiController.siparisHizmetAktifMi),
                        ],
                      );
                    }),
                    CustomTextField(
                    ...
                    CustomTextField(
                        labelText: "Ek Alan 2",
                        onChanged: (p0) =>
                            viewModel.kalemModel.ekalan2 = p0).yetkiVarMi(
                        yetkiController.siparisSatirdaEkAlan2AktifMi),
                    CustomTextField(
                            labelText: "Yapılandırma Kodu",
                            valueWidget: Observer(
                              builder: (_) =>
                                  Text(viewModel.kalemModel.yapkod ?? ""),
                            ),
                            isMust: true,
                            readOnly: true,
                            suffixMore: true,
                            ...
                            onTap: () async {
                              var result = await Get.toNamed(
                                  ...
                                  arguments: widget.stokListesiModel ??
                                      viewModel.model ??
                                      StokListesiModel()
                        ...
                        .yetkiVarMi(widget.stokListesiModel?.yapkod != null ||
                            widget.kalemModel?.yapkod != null),
                    Row(
                      ...
                      children: [
                        Expanded(
                            ...
                            yetkiController.siparisSatirdaTeslimTarihiSor),
                        Expanded(
                            child: CustomTextField(
                          labelText: "Koşul",
                          readOnly: true,
                          ...
                          valueWidget: Observer(
                              builder: (_) =>
                                  Text(viewModel.kalemModel.kosulKodu ?? "")),
                          controller: kosulController,
                          onTap: () async {
                            var result = await bottomSheetDialogManager
                                .showKosullarBottomSheetDialog(context);
                            if (result != null) {
                        ...
                        Expanded(
                          ...
                          valueWidget: Observer(
                              builder: (_) => Text(viewModel
                                      .kalemModel.depoKodu.toStringIfNotNull ??
                                  "")),
                          onTap: () async {
                            var result = await bottomSheetDialogManager
                                .showDepoBottomSheetDialog(context);
                            if (result != null) {
                              depoController.text = result.depoTanimi ??
                                  result.depoKodu.toStringIfNotNull ??
                                  "";
                            ...
                            yetkiController.lokalDepoUygulamasiAcikMi),
                        Expanded(
                            child: CustomTextField(
                          labelText: "Proje",
                          ...
                          valueWidget: Observer(
                              builder: (_) =>
                                  Text(viewModel.kalemModel.projeKodu ?? "")),
                          onTap: () async {
                            var result = await bottomSheetDialogManager
                      ...
                      children: [
                        Expanded(child: Observer(builder: (_) {
                          return CustomTextField(
                            ...
                            isMust: true,
                            controller:
                                viewModel.koliMi ? null : miktarController,
                            ...
                            suffix: Wrap(children: [
                              IconButton(
                                  ...
                                  onPressed: () => viewModel
                                      .decreaseMiktar(miktarController)),
                              IconButton(
                                  icon: const Icon(Icons.add_outlined),
                                  onPressed: () => viewModel
                                      .increaseMiktar(miktarController)),
                            ]),
                          );
                        })),
                        Expanded(child: Observer(builder: (_) {
                          return CustomTextField(
                            ...
                            suffix: Wrap(children: [
                              IconButton(
                                  icon: const Icon(Icons.remove_outlined),
                                  onPressed: () => viewModel
                                      .decreaseMiktar2(miktar2Controller)),
                              IconButton(
                                  icon: const Icon(Icons.add_outlined),
                                  onPressed: () => viewModel
                                      .increaseMiktar2(miktar2Controller)),
                            ]),
                          );
                        })),
                      ],
                        ...
                        Expanded(
                          ...
                          suffix: Wrap(children: [
                            IconButton(
                                icon: const Icon(Icons.remove_outlined),
                                onPressed: () => viewModel
                                    .decreaseMFMiktar(malFazMiktarController)),
                            IconButton(
                                icon: const Icon(Icons.add_outlined),
                                onPressed: () => viewModel
                                    .increaseMFMiktar(malFazMiktarController)),
                          ]),
                        )),
                        Expanded(
                            child: CustomTextField(
                                ...
                                valueWidget: Observer(
                                    builder: (_) => Text(viewModel.kalemModel
                                            .olcuBirimKodu.toStringIfNotNull ??
                                        "")),
                                          ...
                                          children: List.generate(
                                              ...
                                              (index) => BottomSheetModel(
                                                  title: viewModel
                                                      .olcuBirimiMap[index],
                                                  description:
                                                      index.toStringIfNotNull,
                                                  value: MapEntry<String, int>(
                                                      viewModel
                                                          .olcuBirimiMap[index],
                                                      index))));
                                  if (result != null &&
                                      result is MapEntry<String, int>) {
                      ...
                      return Row(
                        ...
                        children: [
                          Expanded(
                            ...
                            suffixMore: true,
                            onClear: () =>
                                viewModel.kalemModel.dovizTipi = null,
                            onTap: () async {
                              var result = await bottomSheetDialogManager
                                  .showDovizBottomSheetDialog(context);
                              if (result != null) {
                                ...
                                viewModel.kalemModel.dovizTipi =
                                    result.dovizKodu;
                              ...
                              widget.stokListesiModel?.dovizliMi ?? false),
                          Expanded(
                              child: CustomTextField(
                            ...
                            onChanged: (value) {
                              viewModel.kalemModel.dovizKuru =
                                  value.toDoubleWithFormattedString;
                              viewModel.setBrutFiyat(
                                  (viewModel.kalemModel.dovizFiyati ?? 0) *
                                      (viewModel.kalemModel.dovizKuru ?? 1));
                              fiyatController.text = viewModel
                                  .kalemModel.brutFiyat
                                  .commaSeparatedWithDecimalDigits(
                                      OndalikEnum.dovizFiyati);
                            },
                          )).yetkiVarMi(
                              (widget.stokListesiModel?.dovizliMi ?? false) &&
                                  viewModel.showDovizBilgileri),
                        ],
                      );
                    }),
                    Observer(
                        builder: (_) => CustomTextField(
                            ...
                            onChanged: (p0) {
                              ...
                              fiyatController.text = viewModel
                                  ...
                                  .commaSeparatedWithDecimalDigits(
                                      OndalikEnum.dovizFiyati);
                            }).yetkiVarMi((widget.stokListesiModel?.dovizliMi ??
                                false) &&
                            viewModel.showDovizBilgileri)),
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Expanded(
                          ...
                          onChanged: (p0) => viewModel
                              .setKdvOrani(p0.toDoubleWithFormattedString),
                          suffix: IconButton(
                            icon: const Icon(Icons.more_horiz_outlined),
                            onPressed: () async {
                              var result = await bottomSheetDialogManager
                                  .showKdvOranlariBottomSheetDialog(context);
                              if (result != null) {
                                viewModel.setKdvOrani(result);
                                kdvOraniController.text =
                                    result.toIntIfDouble.toStringIfNotNull ??
                                        "";
                                  ...
                                  if (viewModel.model?.dovizliMi ?? false) {
                                    viewModel.setDovizFiyati(
                                        (viewModel.kalemModel.brutFiyat ?? 0) /
                                            (viewModel.kalemModel.dovizKuru ??
                                                1));
                                    dovizFiyatiController.text = viewModel
                                        .kalemModel.dovizFiyati
                                        .commaSeparatedWithDecimalDigits(
                                            OndalikEnum.dovizFiyati);
                                  }
                                })),
                    ...
                    ...List.generate(
                        yetkiController.siparisSatirKademeliIskontoSayisi > 6
                            ? 6
                            : yetkiController.siparisSatirKademeliIskontoSayisi,
                        (index) {
                      return Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Expanded(
                              child: CustomTextField(
                                  ...
                                  keyboardType: TextInputType.number,
                                  isFormattedString: true,
                                  suffix: yetkiController
                                              .siparisMSISk1YuzdeSor &&
                                          index == 0
                                      ? Observer(builder: (_) {
                                          return IconButton(
                                              onPressed: () =>
                                                  viewModel.setIskonto1OranMi(),
                                              icon: Icon((viewModel.kalemModel
                                                          .iskonto1OranMi ??
                                                      false)
                    ...
                    Text("Ek Açıklamalar",
                        ...
                        .yetkiVarMi(yetkiController
                            .siparisMSSatirAciklamaAlanlari(null)),
                    CustomTextField(
                        labelText: getAciklamaLabel(1),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama1).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(1)),
                    CustomTextField(
                        labelText: getAciklamaLabel(2),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama2).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(2)),
                    CustomTextField(
                        labelText: getAciklamaLabel(3),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama3).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(3)),
                    CustomTextField(
                        labelText: getAciklamaLabel(4),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama4).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(4)),
                    CustomTextField(
                        labelText: getAciklamaLabel(5),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama5).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(5)),
                    CustomTextField(
                        labelText: getAciklamaLabel(6),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama6).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(6)),
                    CustomTextField(
                        labelText: getAciklamaLabel(7),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama7).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(7)),
                    CustomTextField(
                        labelText: getAciklamaLabel(8),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama8).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(8)),
                    CustomTextField(
                        labelText: getAciklamaLabel(9),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama9).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(9)),
                    CustomTextField(
                        labelText: getAciklamaLabel(10),
                        onChanged: (value) =>
                            viewModel.kalemModel.aciklama10).yetkiVarMi(
                        yetkiController.siparisMSSatirAciklamaAlanlari(10)),
  ...
  String getAciklamaLabel(int index) =>
      "${StaticVariables.instance.isMusteriSiparisleri ? "MS" : "SS"} $index";
  ...
  Future<void> getDovizData() async {
    if (viewModel.dovizliMi) {
      ...
      if (dovizResult.data != null && viewModel.kalemModel.dovizTipi != null) {
        List<DovizKurlariModel> list = dovizResult.data
            .map((e) => e as DovizKurlariModel)
            .toList()
            .cast<DovizKurlariModel>();
        var result = list.firstWhereOrNull(
        ...
        if (result != null) {
          ...
          dovizKuruController.text = viewModel.kalemModel.dovizKuru
              .commaSeparatedWithDecimalDigits(OndalikEnum.dovizFiyati);
          viewModel.kalemModel.dovizFiyati =
              (viewModel.kalemModel.brutFiyat ?? 0) /
                  (viewModel.kalemModel.dovizKuru ?? 1);
          dovizFiyatiController.text = viewModel.kalemModel.dovizFiyati
  ...
  Future<void> controllerFiller() async {
    viewModel.kalemModel.kalemList ??= widget.stokListesiModel?.stokList
        ?.map((e) => KalemModel.fromStokList(e))
        .toList();
    viewModel.kalemModel.stokKodu ??=
        widget.stokListesiModel?.stokKodu ?? widget.kalemModel?.stokKodu;
    viewModel.kalemModel.stokSatDovizAdi ??=
        widget.stokListesiModel?.satisDovizAdi ??
            widget.kalemModel?.stokSatDovizAdi;
    viewModel.kalemModel.stokAlisDovizAdi ??=
        widget.stokListesiModel?.alisDovizAdi ??
            widget.kalemModel?.stokAlisDovizAdi;
    viewModel.kalemModel.stokSatDovTip ??=
        widget.stokListesiModel?.satDovTip ?? viewModel.model?.satDovTip;
    viewModel.setYapKod(
        widget.stokListesiModel?.yapkod ?? widget.kalemModel?.yapkod);
    viewModel.kalemModel.stokAlisDovTip ??= widget.stokListesiModel?.alisDovTip;
    viewModel.kalemModel.dovizTipi ??=
        StaticVariables.instance.isMusteriSiparisleri
            ? viewModel.model?.satDovTip
            : viewModel.model?.alisDovTip;
    kalemAdiController.text = widget.stokListesiModel?.stokAdi ??
        widget.stokListesiModel?.stokKodu ??
        widget.kalemModel?.stokAdi ??
        widget.kalemModel?.stokKodu ??
        "";
    ekAlan1Controller.text = widget.kalemModel?.ekalan1 ?? "";
    ekAlan2Controller.text = widget.kalemModel?.ekalan2 ?? "";
    yapKodController.text = widget.stokListesiModel?.yapkodAciklama ??
        widget.stokListesiModel?.yapkod ??
        widget.kalemModel?.yapkod ??
        "";
    isk1Controller?.text =
        widget.kalemModel?.iskonto1.toIntIfDouble.toStringIfNotNull ?? "";
    isk1TipiController?.text =
        getIskTipiAciklama(widget.kalemModel?.isk1Tipi.toIntIfDouble);
    isk2YuzdeController?.text =
        widget.kalemModel?.iskonto2.toIntIfDouble.toStringIfNotNull ?? "";
    isk2TipiController?.text =
        getIskTipiAciklama(widget.kalemModel?.isk2Tipi.toIntIfDouble);
    isk3YuzdeController?.text =
        widget.kalemModel?.iskonto3.toIntIfDouble.toStringIfNotNull ?? "";
    isk3TipiController?.text =
        getIskTipiAciklama(widget.kalemModel?.isk3Tipi.toIntIfDouble);
    isk4YuzdeController?.text =
        widget.kalemModel?.iskonto4.toIntIfDouble.toStringIfNotNull ?? "";
    isk4TipiController?.text =
        getIskTipiAciklama(widget.kalemModel?.isk4Tipi.toIntIfDouble);
    isk5YuzdeController?.text =
        widget.kalemModel?.iskonto5.toIntIfDouble.toStringIfNotNull ?? "";
    isk5TipiController?.text =
        getIskTipiAciklama(widget.kalemModel?.isk5Tipi.toIntIfDouble);
    isk6YuzdeController?.text =
        widget.kalemModel?.iskonto6.toIntIfDouble.toStringIfNotNull ?? "";
    isk6TipiController?.text =
        getIskTipiAciklama(widget.kalemModel?.isk6Tipi.toIntIfDouble);
    fiyatController.text = (widget.kalemModel?.brutFiyat.toIntIfDouble ??
                viewModel.model?.bulunanFiyat.toIntIfDouble)
            ?.commaSeparatedWithDecimalDigits(OndalikEnum.tutar) ??
        "";
    miktarController.text =
        viewModel.kalemModel.miktar?.toIntIfDouble.toStringIfNotNull ?? "";
    miktar2Controller.text =
        viewModel.kalemModel.miktar2?.toIntIfDouble.toStringIfNotNull ?? "";
    malFazMiktarController.text = viewModel
            .kalemModel.malFazlasiMiktar?.toIntIfDouble.toStringIfNotNull ??
        "";
    olcuBirimiController.text = widget.stokListesiModel?.olcuBirimi ??
        widget.kalemModel?.olcuBirimAdi ??
        "";
    kdvOraniController.text = widget.kalemModel?.kdvOrani
            .commaSeparatedWithDecimalDigits(OndalikEnum.oran) ??
        (StaticVariables.instance.isMusteriSiparisleri
                ? (widget.stokListesiModel?.satisKdv ?? 0)
                : (widget.stokListesiModel?.alisKdv ?? 0))
            .commaSeparatedWithDecimalDigits(OndalikEnum.oran);
    // projeController.text = teslimTarihiController.text = model.teslimTarihi.toDateString;
    kosulController.text =
        model.kosulKodu ?? BaseSiparisEditModel.instance.kosulKodu ?? "";
    dovizTipiController.text = (StaticVariables.instance.isMusteriSiparisleri
            ? viewModel.model?.satisDovizAdi
            : viewModel.model?.alisDovizAdi) ??
        "";
    dovizFiyatiController.text = viewModel.kalemModel.dovizFiyati
        .commaSeparatedWithDecimalDigits(OndalikEnum.dovizTutari);
    ...
    viewModel.kalemModel.stokAdi = widget.stokListesiModel?.stokAdi ??
        widget.stokListesiModel?.stokKodu ??
        widget.kalemModel?.stokAdi ??
        widget.kalemModel?.stokKodu ??
        "";
    ...
    viewModel.kalemModel.teslimTarihi =
        yetkiController.siparisSatirdaTeslimTarihiSor
            ? BaseSiparisEditModel.instance.teslimTarihi
            : null;
    viewModel
        .setShowDovizBilgileri(widget.stokListesiModel?.dovizliMi ?? false);
    viewModel.setOlcuBirimi(MapEntry<String, int>(
        widget.stokListesiModel?.olcuBirimi ??
            widget.kalemModel?.olcuBirimAdi ??
            "",
        0));
    ...
    viewModel.kalemModel.depoKodu ??= BaseSiparisEditModel.instance.topluDepo ??
        viewModel.model?.depoKodu ??
        parametreModel.satisHizmetDepoKodu;
  ...
  String getIskTipiAciklama(num? value) =>
      parametreModel.listIskTip
          ?.firstWhereOrNull((element) => element.iskontoTipi == value)
          ?.aciklama ??
      "";
  ...
  String get getDovizAdi => StaticVariables.instance.isMusteriSiparisleri
      ? (viewModel.kalemModel.stokSatDovizAdi ?? "")
      : (viewModel.kalemModel.stokAlisDovizAdi ?? "");
