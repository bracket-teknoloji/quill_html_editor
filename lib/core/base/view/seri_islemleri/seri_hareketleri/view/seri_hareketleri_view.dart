...
final class _SeriHareketleriViewState extends BaseState<SeriHareketleriView> {
  ...
  final SeriHareketleriViewModel viewModel = SeriHareketleriViewModel();
  ...
  Widget build(BuildContext context) =>
      BaseScaffold(appBar: appBar(context), floatingActionButton: fab(), body: body().paddingAll(UIHelper.lowSize));
              ...
              BottomSheetModel(
                ...
                onTap: () async {
                  ...
                  final result = await networkManager.getStokModel(
                    StokRehberiRequestModel(stokKodu: widget.model?.stokKodu),
                  );
  ...
  Widget fab() => CustomFloatingActionButton(
    ...
    onPressed: () async {
      ...
      final result = await Get.toNamed(
        "/seriGirisi",
        arguments: SeriHareketleriModel.ekle(viewModel.stokListesiModel!),
      );
      if (result == true) {
        await viewModel.getData();
      }
      ...
      Expanded(
        child: RefreshIndicator.adaptive(
          ...
          child: Observer(
            builder: (_) {
              if (viewModel.stokListesiModel == null) {
                return const Center(child: Text("Stok Kodu Seçiniz"));
              }
  ...
  void islemHataDialog() =>
      dialogManager.showAlertDialog("Sadece kayıt tipi D-Devir olan kayıtlara işlem yapılabilir.");
}
