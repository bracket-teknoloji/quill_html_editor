...
final class _DovizKuruGirisiViewState extends BaseState<DovizKuruGirisiView> {
  ...
  void initState() {
    ...
    alisController = TextEditingController(
      text: widget.dovizKurlariModel?.dovAlis?.commaSeparatedWithDecimalDigits(OndalikEnum.dovizFiyati) ?? "",
    );
    satisController = TextEditingController(
      text: widget.dovizKurlariModel?.dovSatis?.commaSeparatedWithDecimalDigits(OndalikEnum.dovizFiyati) ?? "",
    );
    efektifAlisController = TextEditingController(
      text: widget.dovizKurlariModel?.effAlis?.commaSeparatedWithDecimalDigits(OndalikEnum.dovizFiyati) ?? "",
    );
    efektifSatisController = TextEditingController(
      text: widget.dovizKurlariModel?.effSatis?.commaSeparatedWithDecimalDigits(OndalikEnum.dovizFiyati) ?? "",
    );
    super.initState();
  }
  ...
  Future<void> postData() async {
    final result = await networkManager.dioPost(
      ...
      data: [DovizKurlariRequestModel.fromDovizKurlariModel(viewModel.dovizKurlariModel!)],
    );
    if (result.isSuccess) {
