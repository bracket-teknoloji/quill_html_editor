...
class _SplashAuthViewState extends BaseState<SplashAuthView> {
  ...
  Future<void> login() async {
    ...
    if (CacheManager.getVerifiedUser.username == null) {
      ...
      final TokenModel? response = await networkManager.getToken(path: ApiUrls.token, queryParameters: <String, dynamic>{
      ...
      if (response != null && response.error == null) {
        if (response.accessToken != null) {
          await CacheManager.setVerifiedUser(CacheManager.getVerifiedUser);
          await CacheManager.setToken(response.accessToken!);
          await getSession();
        } else {
          await Get.offAllNamed("/login");
        }
  ...
  Future<void> getSession() async {
    ...
    final GenericResponseModel<NetworkManagerMixin> response =
        await networkManager.dioPost<MainPageModel>(path: ApiUrls.createSession, bodyModel: MainPageModel(), data: AccountModel.instance, headers: <String, String>{
    ...
    if (response.data != null) {
      await CacheManager.setAnaVeri(response.data.first);
      await Get.offAllNamed("/mainPage");
      response.message.ext.isNotNullOrNoEmpty ? await dialogManager.showAlertDialog(response.message.toString()) : null;
      final GenericResponseModel<NetworkManagerMixin> result =
          await networkManager.dioPost<AccountModel>(path: ApiUrls.saveUyeBilgileri, showError: false, bodyModel: AccountModel(), data: CacheManager.getHesapBilgileri?.toJson());
      ...
      await Get.offAllNamed("/entryCompany", arguments: true);
    }
  }
}
