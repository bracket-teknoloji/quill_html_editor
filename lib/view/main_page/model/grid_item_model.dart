...
class GridItemModel {
  ...
  bool get yetkiKontrol {
    if (menuTipi == "A") {
      int sayac = 0;
      if ((_menuList?.contains(name) ?? false) && altMenuVarMi && (yetkiListesi != null ? !(yetkiListesi?.any((bool? element) => element == false) ?? false) : true)) {
        for (GridItemModel element in altMenuler!) {
      ...
      altMenuler?.forEach((GridItemModel element) {
        if (!element.yetkiKontrol) {
          sayac--;
        }
      });
