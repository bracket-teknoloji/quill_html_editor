...
final class _MainPageViewState extends BaseState<MainPageView> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      if (CacheManager.getAccounts(AccountModel.instance.uyeEmail ?? "") case final value?) {
        ...
        if ((value.sozlesmeTeklifGoster ?? false) && AccountModel.instance.adminMi) {
          dialogManager.showInfoMaterialBannerWithAction(
            value.karsilamaBaslik ?? "",
            desc: value.karsilamaMesaji,
            onAction: getTeklif,
          );
        } else {
          ...
          onPressed: () async {
            ...
            final result = await bottomSheetDialogManager.showRadioBottomSheetDialog<String>(
              ...
              children: List.generate(
                routes.length,
                (index) => BottomSheetModel(title: routes[index], value: routes[index]),
              ),
            );
            if (result != null) {
          ...
          child: Row(
            children: [
              (CacheManager.getAnaVeri!.userModel!.admin
                      ? const Icon(Icons.local_police_outlined, color: UIHelper.primaryColor, size: 20)
  ...
  Future<void> getTeklif() async {
    dialogManager.showAreYouSureDialog(
      ...
      title:
          "${CacheManager.getAccounts(AccountModel.instance.uyeEmail ?? "")?.karsilamaMesaji}\n\nTeklif almak ister misiniz?",
