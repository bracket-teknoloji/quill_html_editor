...
class _TemsilciProfilViewState extends BaseState<TemsilciProfilView> {
                      ...
                      children: [
                        Observer(
                            builder: (_) => SlideControllerWidget(
                                ...
                                filterOnChanged: (value) => viewModel
                                    .setIadeDurumuValueList(value ?? 0),
                                childrenValueList:
                                    viewModel.iadeDurumuValueList,
                                groupValue: viewModel.iadeDurumuValueList
                                    .firstWhere((element) => element))),
                        Observer(
                            builder: (_) => SlideControllerWidget(
                                title: "Dönem",
                                childrenTitleList: [
                                  "Şirket Yılı (${DateTime.now().year})",
                                  "Bu Yıl (${DateTime.now().year})",
                                  "Geçen Yıl (${DateTime.now().year - 1})"
                                ],
                                filterOnChanged: (value) =>
                                    viewModel.setDonemValueList(value ?? 0),
                                childrenValueList: viewModel.donemValueList,
                                groupValue: viewModel.donemValueList
                                    .firstWhere((element) => element))),
                        CustomTextField(
                            labelText: "Cari VKN",
                            ...
                            suffix: IconButton(
                                onPressed: () async {
                                  var result = await Get.toNamed(
                                      ...
                                      arguments: true);
                                  if (result != null) {
                                    viewModel.temsilciProfilRequestModel
                                        .cariKodu = result.vergiNumarasi;
                                    cariVKNController.text =
                                        result.vergiNumarasi ?? "";
                                  }
                                },
                                icon: const Icon(Icons.more_horiz_outlined)),
                            onChanged: (p0) => viewModel
                                .temsilciProfilRequestModel.cariVKN = p0),
                        Row(
                          children: [
                            Expanded(
                                  ...
                                  var result = await bottomSheetDialogManager
                                      .showBottomSheetDialog(context,
                                          ...
                                          children: viewModel.aylar
                                              ...
                                              .toList());
                                  if (result != null) {
                                    viewModel.setDonemKodu(
                                        viewModel.aylar.indexOf(result) + 1);
                                  }
                                },
                                icon: const Icon(Icons.more_horiz_outlined),
                                label: Observer(
                                    builder: (_) => Text(viewModel.donem)))
                          ],
                        ),
                        Observer(
                            builder: (_) => SizedBox(
                                ...
                                child: CustomPieChart(
                                    ...
                                    pieChartValue:
                                        viewModel.getPlasiyerToplam))),
                        Observer(
                            builder: (_) => CustomLineChart(
                                lineChartValue: viewModel.getAylikSatislar)),
                      ],
                    ).paddingAll(UIHelper.lowSize)),
                    Card(
                        child: Column(
                      children: [
                        ...
                        Observer(builder: (_) {
                          return CustomLineChart(
                              lineChartValue: viewModel.getAylikAlislar);
                        }),
                      ],
                    ).paddingAll(UIHelper.lowSize)),
                    Card(
                      ...
                      children: [
                        ...
                        Observer(
                            builder: (_) => CustomLineChart(
                                lineChartValue: viewModel.getAylikSiparisler)),
                      ],
                    ).paddingAll(UIHelper.lowSize)),
                    Card(
                        child: Column(
                      children: [
                        ...
                        Observer(
                            builder: (_) => ListTile(
                                title: const Text("Bugün"),
                                trailing: Text(
                                    "${viewModel.getBugunTahsilatlar.commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency"))),
                        Observer(
                            builder: (_) => ListTile(
                                title: const Text("Bu Ay"),
                                trailing: Text(
                                    "${viewModel.getBuAyTahsilatlar.commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency"))),
                        Observer(
                            builder: (_) => ListTile(
                                title: const Text("Geçen Ay"),
                                trailing: Text(
                        ...
                        Observer(
                            builder: (_) => CustomLineChart(
                                lineChartValue: viewModel.getAylikTahsilatlar)),
                      ],
                    ).paddingAll(UIHelper.lowSize))
                  ],
                ).paddingAll(UIHelper.lowSize));
  ...
  Future<void> getData() async {
    var result = await networkManager.dioGet<TemsilciProfilModel>(
    ...
    if (result.data != null) {
      ...
      viewModel.setTemsilciProfilList(result.data
          .map((e) => e as TemsilciProfilModel)
          .toList()
          .cast<TemsilciProfilModel>());
    }
  }
}
