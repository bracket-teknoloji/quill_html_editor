...
class _CariHareketleriViewState extends BaseState<CariHareketleriView> {
            ...
            child: ListView(
                ...
                children: <AppBarButton>[
                  ...
                  AppBarButton(
                      ...
                      onPressed: () async {
                        final siralama = await bottomSheetDialogManager.showRadioBottomSheetDialog(context, title: "Sıralama seçiniz", children: <BottomSheetModel?>[
                        ...
                        if (siralama != viewModel.siralama && siralama != null) {
                          ...
                          return getData().then((List<CariHareketleriModel> value) => viewModel.setCariHareketleri(value));
                        }
                      },
                ...
                ].map((AppBarButton e) => e.paddingAll(UIHelper.lowSize)).toList()),
          ),
        ),
      );
          ...
          builder: (_) => (viewModel.cariHareketleriList.ext.isNullOrEmpty
              ? (viewModel.cariHareketleriList?.isEmpty ?? false)
                  ? Center(
                      ...
                      children: <Widget>[Icon(Icons.crisis_alert_outlined, color: theme.colorScheme.primary), const Text("Cari Hareket Detayı Bulunamadı")],
                    ))
              ...
              : Observer(
                  builder: (_) => ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) => Observer(
                            builder: (_) => Visibility(
                                  ...
                                  child: CariHareketlerCard(
                                    ...
                                    onTap: () {
                                      final List<BottomSheetModel> children2 = <BottomSheetModel>[
                                        ...
                                        BottomSheetModel(
                                            ...
                                            onTap: () async {
                                              ...
                                              final List<NetFectDizaynList>? result = anaVeri?.paramModel?.netFectDizaynList?.where((NetFectDizaynList element) => element.ozelKod == "CariHareket").toList();
                                              ...
                                              if (result.ext.isNotNullOrEmpty) {
                                                if (result!.length == 1) {
                                                  ...
                                                  dizaynList = await bottomSheetDialogManager.showBottomSheetDialog(context,
                                                      ...
                                                      children: result.map((NetFectDizaynList e) => BottomSheetModel(title: e.dizaynAdi ?? "", onTap: () => Get.back(result: e))).toList());
                                                }
                                      ...
                                      // B = Kasa hareketi
                                      // G = Ta
                                      if ((viewModel.cariHareketleriList![index].kasaMi ||
                                              viewModel.cariHareketleriList![index].musteriCekMi ||
                                              viewModel.cariHareketleriList![index].musteriSenediMi) &&
                                        ...
                                        children2.add(BottomSheetModel(
                                            ...
                                            onTap: () async {
                                              ...
                                              final MainPageModel? anaVeri = CacheManager.getAnaVeri();
                                              final List<NetFectDizaynList>? result =
                                                  anaVeri?.paramModel?.netFectDizaynList?.where((NetFectDizaynList element) => element.ozelKod == "TahsilatMakbuzu").toList();
                                              NetFectDizaynList? dizaynList;
                                              if (result.ext.isNotNullOrEmpty) {
                                                if (result!.length == 1) {
                                                  pdfModel.dicParams?.caharInckey = viewModel.cariHareketleriList![index].inckeyno.toStringIfNotNull;
      ...
      builder: (_) => ScrollableWidget(
            ...
            child: SizedBox(
              ...
              child: Row(
                children: <Widget>[
                  Expanded(
                      child: FooterButton(children: <Widget>[
                    const Text("Borç"),
                    Observer(
  ...
  Future<List<CariHareketleriModel>> getData() async {
    final GenericResponseModel<NetworkManagerMixin> response = await networkManager.dioGet<CariHareketleriModel>(
      path: ApiUrls.getCariHareketleri,
      bodyModel: CariHareketleriModel(),
      queryParameters: <String, dynamic>{"SIRALAMA": viewModel.siralama, "EkranTipi": "L", "CariKodu": widget.cari?.cariKodu ?? ""},
