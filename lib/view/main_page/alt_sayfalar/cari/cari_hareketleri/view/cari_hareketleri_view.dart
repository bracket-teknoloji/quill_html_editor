            ...
            return Row(
              children: [
                Expanded(
                  ...
                  const Text("Borç"),
                  Observer(builder: (_) {
                    return Text("${(viewModel.borclarToplami).commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} ${widget.cari?.dovizAdi ?? mainCurrency}");
                  })
                ])),
                const VerticalDivider(thickness: 1, width: 1),
                Expanded(
                    child: FooterButton(
                        children: [const Text("Alacak"), Text("${(viewModel.alacaklarToplami).commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} ${widget.cari?.dovizAdi ?? mainCurrency}")])),
                const VerticalDivider(thickness: 1, width: 1),
                Expanded(
                    child: FooterButton(children: [
