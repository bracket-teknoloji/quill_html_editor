...
final class _CariAktiviteGenelViewState extends BaseState<CariAktiviteGenelView> {
  ...
  void initState() {
    ...
    aktiviteTipiController = TextEditingController(
      text: viewModel.model.aktiviteAdi ?? viewModel.model.aktiviteTipi.toStringIfNotNull,
    );
    ...
    sureController = TextEditingController(
      text: viewModel.model.sure.commaSeparatedWithDecimalDigits(OndalikEnum.oran),
    );
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      if (widget.model.baseEditEnum.ekleMi && viewModel.model.kullaniciTitle == null) {
        ...
        viewModel.setKullanici(
          KullanicilarModel(adi: kullaniciController.text, kodu: CacheManager.getAnaVeri?.userModel?.kuladi),
        );
      }
      if (widget.model.baseEditEnum?.ekleMi == true &&
          viewModel.model.aktiviteTipi == null &&
      ...
      child: Column(
        children: [
          ...
          CustomTextField(
            ...
            suffix: IconButton(
              onPressed: () async {
                final result = await bottomSheetDialogManager.showCariAktiviteAciklamalarBottomSheetDialog(
                  context,
                  viewModel.model.aciklama,
                );
                if (result != null) {
