...
class _CariYeniKayitViewState extends BaseState<CariYeniKayitView> {
  ...
  @override
  void initState() {
    viewModel.model = CariHareketYeniKayitModel().fromJson(model?.toJson() ?? <String, dynamic>{});
            ...
            children: <Widget>[
              Observer(
                builder: (_) => ToggleButtons(
                  ...
                  onPressed: (int index) => widget.model?.baseEditEnum == BaseEditEnum.goruntule ? null : viewModel.setIsSelected(index),
                ),
              ...
              Observer(
                  builder: (_) => CustomTextField(
                        ...
                        suffixMore: true,
                        onTap: () async {
                          final Map? result = await bottomSheetDialogManager.showBottomSheetDialog(context, title: "Hareket Türü", children: <BottomSheetModel>[
                            BottomSheetModel(title: "Devir", description: "A", onTap: () => Get.back(result: <String, String>{"title": "Devir", "value": "A"}))
                          ]);
                          if (result != null) {
              ...
              CustomTextField(enabled: enable, labelText: "Belge No", controller: belgeNoController, onChanged: (String value) => viewModel.model.belgeNo = value),
              CustomTextField(enabled: enable, labelText: "Açıklama", controller: aciklamaController, onChanged: (String value) => viewModel.model.aciklama = value),
              Row(
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: CustomTextField(
                        ...
                        onChanged: (String value) => viewModel.model.borc == null ? viewModel.model.alacak = double.tryParse(value) : viewModel.model.borc = double.tryParse(value)),
                  ),
                ],
              ),
              Row(
                      ...
                      onTap: () async {
                        final result = await bottomSheetDialogManager.showBottomSheetDialog(context,
                            ...
                            children: mainPageModel?.paramModel?.plasiyerList
                                ?.map((PlasiyerList e) => BottomSheetModel(title: e.plasiyerAciklama ?? "", description: e.plasiyerKodu, onTap: () => Get.back(result: e)))
                                .toList());
                  ...
                  Expanded(
                    child: CustomTextField(
                      ...
                      onTap: () async {
                        ...
                        final result = await bottomSheetDialogManager.showBottomSheetDialog(context,
                            title: "Projeler", children: viewModel.projeList.map((BaseProjeModel e) => BottomSheetModel(title: e.projeAciklama ?? "", description: e.projeKodu, value: e)).toList());
  ...
  Future<void> postData() async {
    final GenericResponseModel<NetworkManagerMixin> result = await networkManager.dioPost<CariHareketleriModel>(
  ...
  Future<List<BaseProjeModel>?> getProjeData() async {
    ...
    final GenericResponseModel<NetworkManagerMixin> result =
        await networkManager.dioGet<BaseProjeModel>(path: ApiUrls.getProjeler, bodyModel: BaseProjeModel(), addCKey: true, addSirketBilgileri: true);
