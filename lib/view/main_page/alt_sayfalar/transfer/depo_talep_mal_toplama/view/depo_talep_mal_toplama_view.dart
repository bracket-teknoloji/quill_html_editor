...
final class _DepoTalepMalToplamaViewState extends BaseState<DepoTalepMalToplamaView> {
  ...
  AppBar appBar() => AppBar(
    title: AppBarTitle(title: "Depo Talep Mal Toplama", subtitle: "#${model.id.toStringIfNotNull}"),
    actions: [
      IconButton(
        onPressed: () async {
          if (viewModel.model?.kalemler?.any((element) => element.tamamlananMiktar != 0) ?? false) {
            final result = await networkManager.postFatura(
              context,
              SiparisEditRequestModel.forTalepMalToplama(viewModel.model!),
            );
            if (result != null) {
              Get.toNamed(
  ...
  void searchKalem(String value) {
    ...
    if (viewModel.model?.kalemler?.firstWhereOrNull(
          (element) => element.stokKodu?.toUpperCase() == value.toUpperCase(),
        ) ==
        null) {
  ...
  Future<void> editKalem(KalemModel item) async {
    ...
    final result = await Get.toNamed(
      "mainPage/depoMalToplamaKalemEkle",
      arguments: item.copyWith(depoKodu: viewModel.depoList?.depoKodu, depoTanimi: viewModel.depoList?.depoTanimi),
    );
    if (result case final KalemModel value?) {
      final success = await viewModel.saveKalem(value);
      if (success) dialogManager.showSuccessSnackBar("Kalem eklendi");
