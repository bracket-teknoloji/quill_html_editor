...
final class _DepoTalepKalemDetayViewState extends BaseState<DepoTalepKalemDetayView> {
  ...
  AppBar appBar() => AppBar(
    title: AppBarTitle(title: widget.isTalep ? "Depo Talep Detayı" : "Depo Talep Mal Toplama Detayı"),
    actions: [
      IconButton(
        onPressed: () async {
      ...
      child: Column(
        ...
        children: [
          Card(
            child: Column(
              ...
              children: [
                ...
                if (!widget.isTalep) ...[
                  if (widget.model?.miktar case final value?)
                    Text(
                      "Miktar: ${value.commaSeparatedWithDecimalDigits(OndalikEnum.miktar)} ${widget.model?.olcuBirimiAdi}",
                    ),
                  if (widget.model?.tamamlananMiktar case final value?)
                    Text(
                      "Toplanan Miktar: ${value.commaSeparatedWithDecimalDigits(OndalikEnum.miktar)} ${widget.model?.olcuBirimiAdi}",
                    ),
                  if (widget.model?.kalanMiktar case final value?)
                    Text(
                      "Kalan Miktar: ${value.commaSeparatedWithDecimalDigits(OndalikEnum.miktar)} ${widget.model?.olcuBirimiAdi}",
                    ),
                ],
              ],
          ...
          CustomTextField(
            ...
            valueWidget: Observer(builder: (_) => Text(viewModel.model.olcuBirimKodu.toStringIfNotNull ?? "")),
            onClear: () => viewModel.setOlcuBirimi((adi: null, kodu: null)),
            onTap: () async {
              final List<BottomSheetModel> list = List.generate(viewModel.stokModel?.getOlcuBirimiRecords.length ?? 0, (
                index,
              ) {
              ...
              if (list.ext.isNotNullOrEmpty) {
                final result = await bottomSheetDialogManager.showRadioBottomSheetDialog(
                  ...
                  children: list,
                  groupValue: viewModel.model.olcuBirimKodu,
                );
          ...
          CustomTextField(
            ...
            validator: (value) {
              ...
              if (!yetkiController.transferDepoMalToplamaFazlaTeslimatYapabilir) {
                if (value.toDoubleWithFormattedString > (widget.model?.kalanMiktar ?? 0)) {
                  return "Kalan miktarı aşamazsınız";
                }
              }
              return null;
            },
              ...
              children: [
                IconButton(
                  onPressed: () {
                    ...
                    _miktarController.text = ((_miktarController.text.toDoubleWithFormattedString) - 1)
