...
class _StokYeniKayitViewState extends BaseState<StokYeniKayitView> {
  late StokYeniKayitViewModel viewModel = StokYeniKayitViewModel()..isSelected = (widget.model?.cikisIslemi ?? false) ? <bool>[false, true].asObservable() : (<bool>[true, false]).asObservable();
            ...
            child: Column(
              children: <Widget>[
                ...
                CustomTextField(
                  ...
                  onTap: () async {
                    final MapEntry? result = await bottomSheetDialogManager.showBottomSheetDialog(context,
                        title: "Hareket Türü",
                        children: viewModel.hareketTurMap.entries.map((MapEntry<String, String> e) => BottomSheetModel(title: e.key, description: e.value, onTap: () => Get.back(result: e))).toList());
                    if (result != null) {
                  ...
                  onTap: () async {
                    final DepoList? result = await bottomSheetDialogManager.showBottomSheetDialog(context,
                        ...
                        children: viewModel.anaVeri?.paramModel?.depoList
                            ?.map((DepoList e) => BottomSheetModel(title: e.depoTanimi ?? "", description: e.depoKodu.toStringIfNotNull, onTap: () => Get.back(result: e)))
                            .toList());
                    if (result != null) {
                ...
                CustomTextField(
                  ...
                  onSubmitted: (String p0) => viewModel.model.aciklama = p0,
                ),
                CustomTextField(
                  labelText: "Plasiyer",
                  isMust: true,
                  ...
                  onTap: () async {
                    final result = await bottomSheetDialogManager.showBottomSheetDialog(context,
                        ...
                        children: viewModel.anaVeri?.paramModel?.plasiyerList
                            ?.map((PlasiyerList e) => BottomSheetModel(
                ...
                CustomTextField(
                    ...
                    suffixMore: true,
                    onTap: () async {
                      final List<BaseProjeModel>? result = viewModel.projeListesi ?? await getProjeData();
                      if (result != null) {
                        ...
                        final BaseProjeModel? dialogResult = await bottomSheetDialogManager.showBottomSheetDialog(context,
                            ...
                            children: result
                                .map((BaseProjeModel e) => BottomSheetModel(
                                      title: e.projeKodu ?? "",
              ...
              ].map((Widget e) => e.paddingOnly(bottom: UIHelper.lowSize)).toList(),
            ),
          ),
        ),
      );
  ...
  Future<List<BaseProjeModel>?> getProjeData() async {
    ...
    final GenericResponseModel<NetworkManagerMixin> result = await networkManager.dioGet<BaseProjeModel>(path: ApiUrls.getProjeler, bodyModel: BaseProjeModel());
