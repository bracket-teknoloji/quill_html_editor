        ...
        IconButton(
            onPressed: () async {
              await bottomSheetDialogManager.showBottomSheetDialog(context,
                  ...
                  children: [
                    BottomSheetModel(
                        iconWidget: viewModel.dovizliFiyat
                                    ...
                                    child: Observer(builder: (_) {
                                      return ToggleButtons(
                                          constraints: BoxConstraints(
                                              minWidth: (width * 0.9) / 3,
                                              minHeight: height * 0.05),
                                          isSelected: viewModel.isSelected,
                                          onPressed: (index) {
                                            viewModel.changeIsSelected(index);
                                          },
                                          children: viewModel.hareketYonuList
                                              .map((e) => Text(e))
                                              .toList());
                                    }),
                                  )
      ...
      onPressed: () async {
        await Get.toNamed("mainPage/stokYeniKayit",
            arguments: StokHareketleriModel()
              ..stokKodu = widget.model!.stokKodu);
        viewModel.setStokHareketleri(await getData()!);
      },
                              ...
                              child: Card(
                                child: Row(
                                  children: [
                                        ...
                                        children: [
                                          ...
                                          Row(
                                            children: [
                                              Expanded(
                                                  child: Text(
                                                      "Net Fiyat: ${(model.stharNf ?? 0).commaSeparatedWithFixedDigits}")),
                                              Expanded(
                                                  child: Text(
                                                      "Brüt Fiyat: ${(model.stharBf ?? 0).commaSeparatedWithFixedDigits}")),
                                            ],
                                          ),
                                          Row(
                                            children: [
                                              Expanded(
                                                  child: Text(
                                                      "Net Tutar: ${((model.stharNf ?? 0) * (model.stharGcmik ?? 0)).commaSeparatedWithFixedDigits}")),
                                              Expanded(
                                                  child: Text(
                                                      "Brüt Tutar: ${((model.stharBf ?? 0) * (model.stharGcmik ?? 0)).commaSeparatedWithFixedDigits}")),
                                            ],
                                          ),
                                        ],
                                      ).paddingAll(UIHelper.lowSize),
                                    ...
                                    Icon(
                                        model.hareketTuruAciklama != "Muhtelif"
