...
final class DepoBakiyeDurumuView extends StatefulWidget {
...
final class _DepoBakiyeDurumuViewState extends BaseState<DepoBakiyeDurumuView> {
  ...
  Widget build(BuildContext context) =>
      BaseScaffold(appBar: appBar(), bottomNavigationBar: bottomAppBar(), body: body());

  AppBar appBar() => AppBar(
    title: AppBarTitle(title: "Depo Bakiye Durumu", subtitle: widget.model?.stokAdi),
    actions: [
      IconButton(
        onPressed: () async {
  ...
  BottomBarWidget bottomAppBar() => BottomBarWidget(
    ...
    children: [
      FooterButton(
        children: [
          const Text("Giriş"),
          Observer(builder: (_) => Text(viewModel.toplamGiris.commaSeparatedWithDecimalDigits(OndalikEnum.miktar))),
        ],
      ),
      FooterButton(
        children: [
          const Text("Çıkış"),
          Observer(builder: (_) => Text(viewModel.toplamCikis.commaSeparatedWithDecimalDigits(OndalikEnum.miktar))),
        ],
      ),
      FooterButton(
        children: [
          const Text("Bakiye"),
          Observer(builder: (_) => Text(viewModel.toplamBakiye.commaSeparatedWithDecimalDigits(OndalikEnum.miktar))),
        ],
      ),
    ],
  );
  ...
  Observer body() => Observer(
    builder: (_) {
      if (viewModel.depoBakiyeDurumuList == null) {
        ...
        return const Center(child: Text("Depo Bakiye Durumu Bulunamadı"));
      }
