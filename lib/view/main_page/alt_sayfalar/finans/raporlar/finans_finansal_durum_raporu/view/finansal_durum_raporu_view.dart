                ...
                onTap: () async {
                  final result = await bottomSheetDialogManager.showCheckBoxBottomSheetDialog(context,
                      ...
                      children: List.generate(
                        ...
                        (int index) => BottomSheetModel(title: viewModel.gorunecekAlanlarMap.keys.toList()[index], value: viewModel.gorunecekAlanlarMap.entries.toList()[index]),
                      ));

                  if (result is List) {
                    final List<MapEntry<String, String>> items = result.cast<MapEntry<String, String>>();
                    viewModel.changeGorunecekAlanlar(items.map((MapEntry<String, String> e) => e.value).join(";"));
                    gorunecekAlanlarController.text = items.map((MapEntry<String, String> e) => e.key).join(", ");
                  }
                },
              ),
