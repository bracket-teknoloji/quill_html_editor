// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'banka_listesi_model.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

BankaListesiModel _$BankaListesiModelFromJson(Map<String, dynamic> json) {
  return _BankaListesiModel.fromJson(json);
}

/// @nodoc
mixin _$BankaListesiModel {
  String? get bankaKodu => throw _privateConstructorUsedError;
  String? get bankasubeKodu => throw _privateConstructorUsedError;
  String? get hesapKodu => throw _privateConstructorUsedError;
  String? get hesapAdi => throw _privateConstructorUsedError;
  String? get muhasebeHesapTipi => throw _privateConstructorUsedError;
  String? get hesapNo => throw _privateConstructorUsedError;
  String? get ibanNo => throw _privateConstructorUsedError;
  String? get kilit => throw _privateConstructorUsedError;
  int? get hesapTipi => throw _privateConstructorUsedError;
  String? get hesapTipiAdi => throw _privateConstructorUsedError;
  int? get dovizTipi => throw _privateConstructorUsedError;
  double? get borcToplami => throw _privateConstructorUsedError;
  double? get alacakToplami => throw _privateConstructorUsedError;
  double? get dovizborcToplami => throw _privateConstructorUsedError;
  double? get dovizalacakToplami => throw _privateConstructorUsedError;
  String? get dovizAdi => throw _privateConstructorUsedError;
  String? get bankaAdi => throw _privateConstructorUsedError;
  String? get subeAdi => throw _privateConstructorUsedError;
  String? get baglihesapKodu => throw _privateConstructorUsedError;

  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;
  @JsonKey(ignore: true)
  $BankaListesiModelCopyWith<BankaListesiModel> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $BankaListesiModelCopyWith<$Res> {
  factory $BankaListesiModelCopyWith(
          BankaListesiModel value, $Res Function(BankaListesiModel) then) =
      _$BankaListesiModelCopyWithImpl<$Res, BankaListesiModel>;
  @useResult
  $Res call(
      {String? bankaKodu,
      String? bankasubeKodu,
      String? hesapKodu,
      String? hesapAdi,
      String? muhasebeHesapTipi,
      String? hesapNo,
      String? ibanNo,
      String? kilit,
      int? hesapTipi,
      String? hesapTipiAdi,
      int? dovizTipi,
      double? borcToplami,
      double? alacakToplami,
      double? dovizborcToplami,
      double? dovizalacakToplami,
      String? dovizAdi,
      String? bankaAdi,
      String? subeAdi,
      String? baglihesapKodu});
}

/// @nodoc
class _$BankaListesiModelCopyWithImpl<$Res, $Val extends BankaListesiModel>
    implements $BankaListesiModelCopyWith<$Res> {
  _$BankaListesiModelCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? bankaKodu = freezed,
    Object? bankasubeKodu = freezed,
    Object? hesapKodu = freezed,
    Object? hesapAdi = freezed,
    Object? muhasebeHesapTipi = freezed,
    Object? hesapNo = freezed,
    Object? ibanNo = freezed,
    Object? kilit = freezed,
    Object? hesapTipi = freezed,
    Object? hesapTipiAdi = freezed,
    Object? dovizTipi = freezed,
    Object? borcToplami = freezed,
    Object? alacakToplami = freezed,
    Object? dovizborcToplami = freezed,
    Object? dovizalacakToplami = freezed,
    Object? dovizAdi = freezed,
    Object? bankaAdi = freezed,
    Object? subeAdi = freezed,
    Object? baglihesapKodu = freezed,
  }) {
    return _then(_value.copyWith(
      bankaKodu: freezed == bankaKodu
          ? _value.bankaKodu
          : bankaKodu // ignore: cast_nullable_to_non_nullable
              as String?,
      bankasubeKodu: freezed == bankasubeKodu
          ? _value.bankasubeKodu
          : bankasubeKodu // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapKodu: freezed == hesapKodu
          ? _value.hesapKodu
          : hesapKodu // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapAdi: freezed == hesapAdi
          ? _value.hesapAdi
          : hesapAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      muhasebeHesapTipi: freezed == muhasebeHesapTipi
          ? _value.muhasebeHesapTipi
          : muhasebeHesapTipi // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapNo: freezed == hesapNo
          ? _value.hesapNo
          : hesapNo // ignore: cast_nullable_to_non_nullable
              as String?,
      ibanNo: freezed == ibanNo
          ? _value.ibanNo
          : ibanNo // ignore: cast_nullable_to_non_nullable
              as String?,
      kilit: freezed == kilit
          ? _value.kilit
          : kilit // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapTipi: freezed == hesapTipi
          ? _value.hesapTipi
          : hesapTipi // ignore: cast_nullable_to_non_nullable
              as int?,
      hesapTipiAdi: freezed == hesapTipiAdi
          ? _value.hesapTipiAdi
          : hesapTipiAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      dovizTipi: freezed == dovizTipi
          ? _value.dovizTipi
          : dovizTipi // ignore: cast_nullable_to_non_nullable
              as int?,
      borcToplami: freezed == borcToplami
          ? _value.borcToplami
          : borcToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      alacakToplami: freezed == alacakToplami
          ? _value.alacakToplami
          : alacakToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      dovizborcToplami: freezed == dovizborcToplami
          ? _value.dovizborcToplami
          : dovizborcToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      dovizalacakToplami: freezed == dovizalacakToplami
          ? _value.dovizalacakToplami
          : dovizalacakToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      dovizAdi: freezed == dovizAdi
          ? _value.dovizAdi
          : dovizAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      bankaAdi: freezed == bankaAdi
          ? _value.bankaAdi
          : bankaAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      subeAdi: freezed == subeAdi
          ? _value.subeAdi
          : subeAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      baglihesapKodu: freezed == baglihesapKodu
          ? _value.baglihesapKodu
          : baglihesapKodu // ignore: cast_nullable_to_non_nullable
              as String?,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$BankaListesiModelImplCopyWith<$Res>
    implements $BankaListesiModelCopyWith<$Res> {
  factory _$$BankaListesiModelImplCopyWith(_$BankaListesiModelImpl value,
          $Res Function(_$BankaListesiModelImpl) then) =
      __$$BankaListesiModelImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {String? bankaKodu,
      String? bankasubeKodu,
      String? hesapKodu,
      String? hesapAdi,
      String? muhasebeHesapTipi,
      String? hesapNo,
      String? ibanNo,
      String? kilit,
      int? hesapTipi,
      String? hesapTipiAdi,
      int? dovizTipi,
      double? borcToplami,
      double? alacakToplami,
      double? dovizborcToplami,
      double? dovizalacakToplami,
      String? dovizAdi,
      String? bankaAdi,
      String? subeAdi,
      String? baglihesapKodu});
}

/// @nodoc
class __$$BankaListesiModelImplCopyWithImpl<$Res>
    extends _$BankaListesiModelCopyWithImpl<$Res, _$BankaListesiModelImpl>
    implements _$$BankaListesiModelImplCopyWith<$Res> {
  __$$BankaListesiModelImplCopyWithImpl(_$BankaListesiModelImpl _value,
      $Res Function(_$BankaListesiModelImpl) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? bankaKodu = freezed,
    Object? bankasubeKodu = freezed,
    Object? hesapKodu = freezed,
    Object? hesapAdi = freezed,
    Object? muhasebeHesapTipi = freezed,
    Object? hesapNo = freezed,
    Object? ibanNo = freezed,
    Object? kilit = freezed,
    Object? hesapTipi = freezed,
    Object? hesapTipiAdi = freezed,
    Object? dovizTipi = freezed,
    Object? borcToplami = freezed,
    Object? alacakToplami = freezed,
    Object? dovizborcToplami = freezed,
    Object? dovizalacakToplami = freezed,
    Object? dovizAdi = freezed,
    Object? bankaAdi = freezed,
    Object? subeAdi = freezed,
    Object? baglihesapKodu = freezed,
  }) {
    return _then(_$BankaListesiModelImpl(
      bankaKodu: freezed == bankaKodu
          ? _value.bankaKodu
          : bankaKodu // ignore: cast_nullable_to_non_nullable
              as String?,
      bankasubeKodu: freezed == bankasubeKodu
          ? _value.bankasubeKodu
          : bankasubeKodu // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapKodu: freezed == hesapKodu
          ? _value.hesapKodu
          : hesapKodu // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapAdi: freezed == hesapAdi
          ? _value.hesapAdi
          : hesapAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      muhasebeHesapTipi: freezed == muhasebeHesapTipi
          ? _value.muhasebeHesapTipi
          : muhasebeHesapTipi // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapNo: freezed == hesapNo
          ? _value.hesapNo
          : hesapNo // ignore: cast_nullable_to_non_nullable
              as String?,
      ibanNo: freezed == ibanNo
          ? _value.ibanNo
          : ibanNo // ignore: cast_nullable_to_non_nullable
              as String?,
      kilit: freezed == kilit
          ? _value.kilit
          : kilit // ignore: cast_nullable_to_non_nullable
              as String?,
      hesapTipi: freezed == hesapTipi
          ? _value.hesapTipi
          : hesapTipi // ignore: cast_nullable_to_non_nullable
              as int?,
      hesapTipiAdi: freezed == hesapTipiAdi
          ? _value.hesapTipiAdi
          : hesapTipiAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      dovizTipi: freezed == dovizTipi
          ? _value.dovizTipi
          : dovizTipi // ignore: cast_nullable_to_non_nullable
              as int?,
      borcToplami: freezed == borcToplami
          ? _value.borcToplami
          : borcToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      alacakToplami: freezed == alacakToplami
          ? _value.alacakToplami
          : alacakToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      dovizborcToplami: freezed == dovizborcToplami
          ? _value.dovizborcToplami
          : dovizborcToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      dovizalacakToplami: freezed == dovizalacakToplami
          ? _value.dovizalacakToplami
          : dovizalacakToplami // ignore: cast_nullable_to_non_nullable
              as double?,
      dovizAdi: freezed == dovizAdi
          ? _value.dovizAdi
          : dovizAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      bankaAdi: freezed == bankaAdi
          ? _value.bankaAdi
          : bankaAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      subeAdi: freezed == subeAdi
          ? _value.subeAdi
          : subeAdi // ignore: cast_nullable_to_non_nullable
              as String?,
      baglihesapKodu: freezed == baglihesapKodu
          ? _value.baglihesapKodu
          : baglihesapKodu // ignore: cast_nullable_to_non_nullable
              as String?,
    ));
  }
}

/// @nodoc
@JsonSerializable()
class _$BankaListesiModelImpl implements _BankaListesiModel {
  const _$BankaListesiModelImpl(
      {this.bankaKodu,
      this.bankasubeKodu,
      this.hesapKodu,
      this.hesapAdi,
      this.muhasebeHesapTipi,
      this.hesapNo,
      this.ibanNo,
      this.kilit,
      this.hesapTipi,
      this.hesapTipiAdi,
      this.dovizTipi,
      this.borcToplami,
      this.alacakToplami,
      this.dovizborcToplami,
      this.dovizalacakToplami,
      this.dovizAdi,
      this.bankaAdi,
      this.subeAdi,
      this.baglihesapKodu});

  factory _$BankaListesiModelImpl.fromJson(Map<String, dynamic> json) =>
      _$$BankaListesiModelImplFromJson(json);

  @override
  final String? bankaKodu;
  @override
  final String? bankasubeKodu;
  @override
  final String? hesapKodu;
  @override
  final String? hesapAdi;
  @override
  final String? muhasebeHesapTipi;
  @override
  final String? hesapNo;
  @override
  final String? ibanNo;
  @override
  final String? kilit;
  @override
  final int? hesapTipi;
  @override
  final String? hesapTipiAdi;
  @override
  final int? dovizTipi;
  @override
  final double? borcToplami;
  @override
  final double? alacakToplami;
  @override
  final double? dovizborcToplami;
  @override
  final double? dovizalacakToplami;
  @override
  final String? dovizAdi;
  @override
  final String? bankaAdi;
  @override
  final String? subeAdi;
  @override
  final String? baglihesapKodu;

  @override
  String toString() {
    return 'BankaListesiModel(bankaKodu: $bankaKodu, bankasubeKodu: $bankasubeKodu, hesapKodu: $hesapKodu, hesapAdi: $hesapAdi, muhasebeHesapTipi: $muhasebeHesapTipi, hesapNo: $hesapNo, ibanNo: $ibanNo, kilit: $kilit, hesapTipi: $hesapTipi, hesapTipiAdi: $hesapTipiAdi, dovizTipi: $dovizTipi, borcToplami: $borcToplami, alacakToplami: $alacakToplami, dovizborcToplami: $dovizborcToplami, dovizalacakToplami: $dovizalacakToplami, dovizAdi: $dovizAdi, bankaAdi: $bankaAdi, subeAdi: $subeAdi, baglihesapKodu: $baglihesapKodu)';
  }

  @override
  bool operator ==(dynamic other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$BankaListesiModelImpl &&
            (identical(other.bankaKodu, bankaKodu) ||
                other.bankaKodu == bankaKodu) &&
            (identical(other.bankasubeKodu, bankasubeKodu) ||
                other.bankasubeKodu == bankasubeKodu) &&
            (identical(other.hesapKodu, hesapKodu) ||
                other.hesapKodu == hesapKodu) &&
            (identical(other.hesapAdi, hesapAdi) ||
                other.hesapAdi == hesapAdi) &&
            (identical(other.muhasebeHesapTipi, muhasebeHesapTipi) ||
                other.muhasebeHesapTipi == muhasebeHesapTipi) &&
            (identical(other.hesapNo, hesapNo) || other.hesapNo == hesapNo) &&
            (identical(other.ibanNo, ibanNo) || other.ibanNo == ibanNo) &&
            (identical(other.kilit, kilit) || other.kilit == kilit) &&
            (identical(other.hesapTipi, hesapTipi) ||
                other.hesapTipi == hesapTipi) &&
            (identical(other.hesapTipiAdi, hesapTipiAdi) ||
                other.hesapTipiAdi == hesapTipiAdi) &&
            (identical(other.dovizTipi, dovizTipi) ||
                other.dovizTipi == dovizTipi) &&
            (identical(other.borcToplami, borcToplami) ||
                other.borcToplami == borcToplami) &&
            (identical(other.alacakToplami, alacakToplami) ||
                other.alacakToplami == alacakToplami) &&
            (identical(other.dovizborcToplami, dovizborcToplami) ||
                other.dovizborcToplami == dovizborcToplami) &&
            (identical(other.dovizalacakToplami, dovizalacakToplami) ||
                other.dovizalacakToplami == dovizalacakToplami) &&
            (identical(other.dovizAdi, dovizAdi) ||
                other.dovizAdi == dovizAdi) &&
            (identical(other.bankaAdi, bankaAdi) ||
                other.bankaAdi == bankaAdi) &&
            (identical(other.subeAdi, subeAdi) || other.subeAdi == subeAdi) &&
            (identical(other.baglihesapKodu, baglihesapKodu) ||
                other.baglihesapKodu == baglihesapKodu));
  }

  @JsonKey(ignore: true)
  @override
  int get hashCode => Object.hashAll([
        runtimeType,
        bankaKodu,
        bankasubeKodu,
        hesapKodu,
        hesapAdi,
        muhasebeHesapTipi,
        hesapNo,
        ibanNo,
        kilit,
        hesapTipi,
        hesapTipiAdi,
        dovizTipi,
        borcToplami,
        alacakToplami,
        dovizborcToplami,
        dovizalacakToplami,
        dovizAdi,
        bankaAdi,
        subeAdi,
        baglihesapKodu
      ]);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$BankaListesiModelImplCopyWith<_$BankaListesiModelImpl> get copyWith =>
      __$$BankaListesiModelImplCopyWithImpl<_$BankaListesiModelImpl>(
          this, _$identity);

  @override
  Map<String, dynamic> toJson() {
    return _$$BankaListesiModelImplToJson(
      this,
    );
  }
}

abstract class _BankaListesiModel implements BankaListesiModel {
  const factory _BankaListesiModel(
      {final String? bankaKodu,
      final String? bankasubeKodu,
      final String? hesapKodu,
      final String? hesapAdi,
      final String? muhasebeHesapTipi,
      final String? hesapNo,
      final String? ibanNo,
      final String? kilit,
      final int? hesapTipi,
      final String? hesapTipiAdi,
      final int? dovizTipi,
      final double? borcToplami,
      final double? alacakToplami,
      final double? dovizborcToplami,
      final double? dovizalacakToplami,
      final String? dovizAdi,
      final String? bankaAdi,
      final String? subeAdi,
      final String? baglihesapKodu}) = _$BankaListesiModelImpl;

  factory _BankaListesiModel.fromJson(Map<String, dynamic> json) =
      _$BankaListesiModelImpl.fromJson;

  @override
  String? get bankaKodu;
  @override
  String? get bankasubeKodu;
  @override
  String? get hesapKodu;
  @override
  String? get hesapAdi;
  @override
  String? get muhasebeHesapTipi;
  @override
  String? get hesapNo;
  @override
  String? get ibanNo;
  @override
  String? get kilit;
  @override
  int? get hesapTipi;
  @override
  String? get hesapTipiAdi;
  @override
  int? get dovizTipi;
  @override
  double? get borcToplami;
  @override
  double? get alacakToplami;
  @override
  double? get dovizborcToplami;
  @override
  double? get dovizalacakToplami;
  @override
  String? get dovizAdi;
  @override
  String? get bankaAdi;
  @override
  String? get subeAdi;
  @override
  String? get baglihesapKodu;
  @override
  @JsonKey(ignore: true)
  _$$BankaListesiModelImplCopyWith<_$BankaListesiModelImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
