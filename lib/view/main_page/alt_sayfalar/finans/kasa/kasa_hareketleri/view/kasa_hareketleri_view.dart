...
class _KasaHareketleriViewState extends BaseState<KasaHareketleriView> {
                ...
                return Observer(
                    builder: (_) => ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            final KasaIslemleriModel? item = viewModel.kasaIslemleriListesi?[index];
                            return Card(
                              child: ListTile(
                                onTap: () async {
                                  await bottomSheetDialogManager.showBottomSheetDialog(context, title: item?.tipAciklama ?? "", children: <BottomSheetModel>[
                                    BottomSheetModel(
                                        title: "Sil",
                                        ...
                                        onTap: () {
                                          ...
                                          dialogManager.showAreYouSureDialog(() async {
                                            final GenericResponseModel<NetworkManagerMixin> result = await viewModel.deleteData(item?.inckeyno);
                                ...
                                subtitle: Wrap(
                                  ...
                                  ].whereType<Text>().toList().map((Text e) => SizedBox(width: width * 0.5, child: e)).toList(),
                                ),
                                trailing: Text(
  ...
  Observer bottomBar() => Observer(
      builder: (_) => BottomBarWidget(isScrolledDown: viewModel.isScrollDown, children: <FooterButton>[
