    ...
    child: Observer(
      builder: (_) {
        if (viewModel.dekontListesi == null) {
          ...
          return const Center(child: Text("Dekont bulunamadı"));
        }
          ...
          itemBuilder: (context, index) {
            if (index == viewModel.dekontListesi?.length) {
              return Visibility(
                ...
                child: const Center(
                  child: Padding(padding: EdgeInsets.all(8.0), child: CircularProgressIndicator.adaptive()),
                ),
              );
            } else {
              final DekontListesiModel model = viewModel.dekontListesi![index];
              return DekontlarCard(model: model, onSelected: (value) async => await viewModel.resetPage());
            }
          },
        );
      },
