...
class KrediKartiTahsilatiView extends StatefulWidget {
  ...
  @override
  State<KrediKartiTahsilatiView> createState() =>
      _KrediKartiTahsilatiViewState();
}
...
class _KrediKartiTahsilatiViewState extends BaseState<KrediKartiTahsilatiView> {
    ...
    return AppBar(
      title: Observer(
          builder: (_) => AppBarTitle(
              title: "K.Kartı Tahsilatı", subtitle: viewModel.appBarSubTitle)),
      actions: [
        IconButton(
          onPressed: () async {
            ...
            children: [
              ...
              CustomTextField(
                  ...
                  valueWidget: Observer(
                      builder: (_) => Text(viewModel.model.cariKodu ?? "")),
              ...
              Observer(
                  builder: (_) => Text(
                        (viewModel.getCariBakiye ?? "") +
                            ((viewModel.cariBakiye ?? 0) > 0
                                ? " (Tahsil Edilecek)"
                                : " (Ödenecek)"),
                        style: TextStyle(
                            color: (viewModel.cariBakiye ?? 0) > 0
                                ? Colors.green
                                : Colors.red),
                      )
                          .paddingAll(UIHelper.lowSize)
                          .yetkiVarMi(viewModel.getCariBakiye != null)),
              Observer(builder: (_) {
                return Row(
                  children: [
                    Expanded(
                    ...
                    ).yetkiVarMi(viewModel.model.kktYontemi == "K" ||
                        viewModel.model.kktYontemi == "H"),
                    Expanded(
                      child: CustomTextField(
                        labelText: "Sözleşme",
                        ...
                        valueWidget: Observer(
                            builder: (_) =>
                                Text(viewModel.model.sozlesmeKodu ?? "")),
                  ...
                  children: [
                    Expanded(
                      child: CustomTextField(
                        ...
                        valueWidget: Observer(
                            builder: (_) =>
                                Text(viewModel.model.dekontSeri ?? "")),
  ...
  Future<void> tahsilatYontemiDialog() async {
    var result = await bottomSheetDialogManager
        .showBottomSheetDialog(context, title: "Tahsilat Yöntemi", children: [
      BottomSheetModel(
          title: "Hızlı Tahsilat Modülü", value: "Hızlı Tahsilat Modülü"),
    ...
    if (viewModel.bankaHesaplariList.ext.isNotNullOrEmpty) {
      var result = await bottomSheetDialogManager.showRadioBottomSheetDialog(
          ...
          children: viewModel.bankaHesaplariList!
              .map((e) => BottomSheetModel(
                  title: e.hesapAdi ?? "", description: e.hesapKodu, value: e))
              .toList());
    ...
    if (viewModel.seriList.ext.isNotNullOrEmpty) {
      var result = await bottomSheetDialogManager.showRadioBottomSheetDialog(
          ...
          children: viewModel.seriList!
              .map((e) => BottomSheetModel(
                  title: e.aciklama ?? "", description: e.seriNo, value: e))
              .toList());
      if (result != null) {
