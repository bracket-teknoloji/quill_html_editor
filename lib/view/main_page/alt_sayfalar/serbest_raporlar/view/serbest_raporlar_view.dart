...
class _SerbestRaporlarViewState extends BaseState<SerbestRaporlarView> {
  ...
  @override
  Widget build(BuildContext context) => PDFViewerView(filterBottomSheet: filterBottomSheet, title: widget.dizaynList?.dizaynAdi ?? "Serbest Raporlar", pdfData: viewModel.pdfModel);
      ...
      body: Column(
        ...
        children: <Widget>[
          Center(
            child: Wrap(
                children: viewModel.serbestRaporResponseModelList
                        ?.map((SerbestRaporResponseModel e) {
                          if (e.tipi == "Date") {
          ...
          ElevatedButton(
                  onPressed: () {
                    //😳 Düzelt kanki
                    if (viewModel.serbestRaporResponseModelList?.where((SerbestRaporResponseModel element) => element.bosGecilebilir == false).any((SerbestRaporResponseModel element) => viewModel.dicParams[element.adi ?? ""] == null) ?? false) {
  ...
  Future<void> getRehber(SerbestRaporResponseModel model) async {
    if (model.stokKoduMu || model.cariKoduMu) {
      final result = await Get.toNamed("/mainPage/${model.rehberTipi?.toLowerCase()}Listesi", arguments: true);
      if (result != null) {
      ...
      final List<BaseGrupKoduModel> grupKodList = await networkManager.getGrupKod(name: model.rehberTipi?.split("_").first ?? "", grupNo: 0);
      final result = await bottomSheetDialogManager.showBottomSheetDialog(context,
          title: "Grup Kodu Seçiniz", children: grupKodList.map((BaseGrupKoduModel e) => BottomSheetModel(title: e.grupAdi ?? "", onTap: () => Get.back(result: e))).toList());
      ...
      final List<BaseGrupKoduModel> grupKodList = await networkManager.getGrupKod(name: model.rehberTipi?.split("_").first ?? "", grupNo: int.tryParse(model.rehberTipi!.split("").last) ?? 0);
      final result = await bottomSheetDialogManager.showBottomSheetDialog(context,
          title: "Grup Kodu Seçiniz", children: grupKodList.map((BaseGrupKoduModel e) => BottomSheetModel(title: e.grupKodu ?? "", onTap: () => Get.back(result: e))).toList());
      if (result != null) {
        viewModel.changeDicParams(model.adi ?? "", result.grupKodu ?? "");
      }
      ...
      final GenericResponseModel<NetworkManagerMixin> muhasebeList = await networkManager.dioGet<StokMuhasebeKoduModel>(path: ApiUrls.getStokMuhasebeKodlari, bodyModel: StokMuhasebeKoduModel());
      final result = await bottomSheetDialogManager.showBottomSheetDialog(context,
  ...
  Future<void> getData() async {
    ...
    final GenericResponseModel<NetworkManagerMixin> result = await networkManager.dioGet<SerbestRaporResponseModel>(path: ApiUrls.getDizaynParametreleri, bodyModel: SerbestRaporResponseModel(), queryParameters: <String, int?>{"ID": widget.dizaynList?.id});
