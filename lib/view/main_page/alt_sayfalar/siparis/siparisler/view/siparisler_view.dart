    ...
    return Observer(
        builder: (_) => BottomBarWidget(isScrolledDown: viewModel.isScrolledDown, visible: viewModel.paramData.isNotEmpty, children: [
              FooterButton(children: [
                ...
                Text("${int.tryParse(viewModel.paramData["ARA_TOPLAM"]?.split(",").first ?? "").commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency")
              ]),
              FooterButton(children: [const Text("KDV"), Text("${int.tryParse(viewModel.paramData["KDV"]?.split(",").first ?? "").commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency")]),
              FooterButton(children: [
                const Text("KDV Dahil"),
                Text("${int.tryParse(viewModel.paramData["GENEL_TOPLAM"]?.split(",").first ?? "").commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency")
              ]),
            ]));
  }
