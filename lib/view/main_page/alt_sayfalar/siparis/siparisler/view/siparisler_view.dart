...
class _SiparislerViewState extends BaseState<SiparislerView> {
  ...
  List<BaseSiparisEditModel?>? get musteriSiparisleriList =>
      viewModel.musteriSiparisleriList;

  @override
  void initState() {
    viewModel = SiparislerViewModel(
        pickerBelgeTuru:
            widget.widgetModel.siparisTipiEnum == SiparisTipiEnum.musteri
                ? "MS"
                : "SS");
    ...
    return AppBar(
      ...
      title: Observer(
          builder: (_) => viewModel.searchBar
              ...
              : AppBarTitle(
                  title:
                      "${StaticVariables.instance.isMusteriSiparisleri ? "Müşteri" : "Satıcı"} Siparişleri",
                  subtitle:
                      viewModel.musteriSiparisleriList?.length.toString())),
      //*Actions
      actions: [
        IconButton(
            onPressed: () {
                      ...
                      children: [
                                ...
                                child: CustomTextField(
                                    ...
                                    onTap: () async {
                                      var result = await bottomSheetDialogManager
                                          .showPlasiyerListesiBottomSheetDialog(
                                              context);
                                      if (result.ext.isNotNullOrEmpty) {
                                        plasiyerController.text = result!
                                            .map((e) => e?.plasiyerAciklama)
                                            .join(", ");

                                        viewModel.setArrPlasiyerKodu(result
                                            .map((e) =>
                                                e?.plasiyerKodu.toString())
                                            .toList()
                                            .cast<String>());
                                      }
                                    })),
                            Expanded(
                                child: CustomTextField(
                                    ...
                                    onTap: () async {
                                      var result =
                                          await bottomSheetDialogManager
                                              .showProjeBottomSheetDialog(
                                                  context);
                                      if (result != null) {
                                        projeController.text =
                                            result.projeAciklama ?? "";
                                        viewModel
                                            .setProjeKodu(result.projeKodu);
                                      }
                                    })),
                          ],
                        ),
                        ...
                        Observer(
                            builder: (_) => SlideControllerWidget(
                                ...
                                filterOnChanged: (x) =>
                                    viewModel.setTeslimatDurumuGroupValue(x),
                                childrenValueList:
                                    viewModel.teslimatDurumuValueList,
                                groupValue:
                                    viewModel.teslimatDurumuGroupValue)),
                        InkWell(
                          onTap: () => viewModel.changeGrupKodlariGoster(),
                          child: const Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text("Cari Rapor Kodları"),
                                ...
                                Row(
                                  children: [
                                    Expanded(
                                      ...
                                      suffixMore: true,
                                      onTap: () =>
                                          getGrupKodu(0, grupKoduController),
                                  ...
                                  Observer(
                                      builder: (_) => SwitchListTile.adaptive(
                                          ...
                                          value: viewModel
                                              ...
                                              false,
                                          onChanged: (value) =>
                                              viewModel.changeEkstraAlanlarMap(
                                                  "EK", value))),
                                  Observer(
                                      builder: (_) => SwitchListTile.adaptive(
                                          title: const Text("Miktar"),
                                          value: viewModel
                                                  .ekstraAlanlarMap["MİK"] ??
                                              false,
                                          onChanged: (value) {
                                            viewModel.changeEkstraAlanlarMap(
                                                "MİK", value);
            ...
            builder: (_) => musteriSiparisleriList.ext.isNullOrEmpty
                ...
                : Observer(builder: (_) {
                    return ListView.builder(
                      ...
                      itemCount: musteriSiparisleriList?.length != null
                          ? musteriSiparisleriList!.length + 1
                          : 0,
                      itemBuilder: (context, index) {
                        ...
                        return Observer(builder: (_) {
                          return SiparislerCard(
                            ...
                            showEkAciklama:
                                viewModel.ekstraAlanlarMap["EK"] ?? false,
                            showMiktar:
                                viewModel.ekstraAlanlarMap["MİK"] ?? false,
                            showVade:
                                viewModel.ekstraAlanlarMap["VADE"] ?? false,
                            model: viewModel.musteriSiparisleriList?[index] ??
                                BaseSiparisEditModel(),
                            index: (viewModel.musteriSiparisleriList?[index]
                                        ?.isNew ??
                                    false)
                                ? index
                                : null,
  ...
  Observer bottomNavigationBar() {
    return Observer(
        builder: (_) => BottomBarWidget(
                isScrolledDown: viewModel.isScrolledDown,
                visible: viewModel.paramData.isNotEmpty,
                children: [
                  FooterButton(children: [
                    const Text("KDV Hariç"),
                    Text(
                        "${int.tryParse(viewModel.paramData["ARA_TOPLAM"]?.split(",").first ?? "").commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency")
                  ]),
                  FooterButton(children: [
                    const Text("KDV"),
                    Text(
                        "${int.tryParse(viewModel.paramData["KDV"]?.split(",").first ?? "").commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency")
                  ]),
                  FooterButton(children: [
                    const Text("KDV Dahil"),
                    Text(
                        "${int.tryParse(viewModel.paramData["GENEL_TOPLAM"]?.split(",").first ?? "").commaSeparatedWithDecimalDigits(OndalikEnum.tutar)} $mainCurrency")
                  ]),
                ]));
  }
  ...
  void getData() async {
    if (viewModel.grupKodList.ext.isNullOrEmpty) {
      viewModel.changeGrupKodList(
          await networkManager.getGrupKod(name: "CARI", grupNo: -1));
    }
    ...
    if (result.data != null) {
      if (viewModel.sayfa == 1) {
        viewModel.setSiparislerList(
            CacheManager.getSiparisEditLists(widget.widgetModel.siparisTipiEnum)
                ?.toList()
                .cast<BaseSiparisEditModel?>());
        viewModel.setParamData(result.paramData
                ?.map((key, value) => MapEntry(key, value.toString())) ??
            {});
      }
      List<BaseSiparisEditModel?>? list = result.data
  ...
  Future<String?> getGrupKodu(
    ...
    List<BottomSheetModel>? bottomSheetList = viewModel.grupKodList
        .where((e) => e.grupNo == grupNo)
        .toList()
        .cast<BaseGrupKoduModel>()
        .map((e) => BottomSheetModel(title: e.grupKodu ?? ""))
        .toList()
    ...
    if (result != null) {
      ...
      switch (grupNo) {
        case 0:
          viewModel.setArrGrupKodu(
              result.map((e) => e.toString()).toList().cast<String>() ?? "");
          break;
        case 1:
          viewModel.setArrKod1(
              result.map((e) => e.toString()).toList().cast<String>() ?? "");
          break;
        case 2:
          viewModel.setArrKod2(
              result.map((e) => e.toString()).toList().cast<String>() ?? "");
          break;
        case 3:
          viewModel.setArrKod3(
              result.map((e) => e.toString()).toList().cast<String>() ?? "");
          break;
        case 4:
          viewModel.setArrKod4(
              result.map((e) => e.toString()).toList().cast<String>() ?? "");
          break;
        case 5:
          viewModel.setArrKod5(
              result.map((e) => e.toString()).toList().cast<String>() ?? "");
          break;
      }
    }
