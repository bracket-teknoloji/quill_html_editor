...
class BaseSiparisEditModel with NetworkManagerMixin {
  ...
  double get genelToplamTutar {
    genelToplam = getAraToplam + ((kdvDahilMi ?? false) ? 0 : kdvTutari);
  ...
  double get getDovizliAraToplam => kalemList?.map((e) => e.getDovizAraToplamTutari).toList().fold(0, (a, b) => (a ?? 0) + b) ?? 0;
  ...
  double get getToplamIskonto => toplamBrutTutar - getAraToplam - ((kdvDahilMi ?? false) ?0: kdvTutari);
...
class KalemModel with NetworkManagerMixin {
  ...
  double toplamKalemMiktari([bool miktar2EklensinMi = false]) => ((miktar ?? (miktar2EklensinMi ? miktar2 : null)) ?? 0) + (malFazlasiMiktar ?? 0);
  ...
  double get dovizliBrutTutar => ((getSelectedMiktar ?? 0) + (malFazlasiMiktar ?? 0)) * (dovizFiyati ?? 0);
  ...
  double get getAraToplamTutari => araToplamTutari - ((BaseSiparisEditModel.instance.kdvDahilMi ?? false) ?0: kdvTutari);
