...
class _AddAccountViewState extends BaseState<AddAccountView> {
  ...
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(
          ...
          centerTitle: false,
          actions: <Widget>[IconButton(onPressed: loginMethod, icon: const Icon(Icons.save))],
        ),
                  ...
                  children: <Widget>[
                    ...
                    Padding(
                      ...
                      child: CustomWidgetWithLabel(
                        text: "Şifre",
                        onlyLabelpaddingLeft: UIHelper.lowSize,
                        child: CustomTextField(keyboardType: TextInputType.visiblePassword, controller: passwordController, isMust: true, onSubmitted: (String value) => loginMethod),
                      ),
                    ),
                    const Wrap(
                      direction: Axis.horizontal,
                      crossAxisAlignment: WrapCrossAlignment.center,
                      children: <Widget>[Icon(Icons.question_mark_rounded), Text("Bilgileri girerken büyük-küçük uyumuna dikkat ediniz.", softWrap: true)],
                    ),
  ...
  Future<void> loginMethod() async {
    if (formKey.currentState?.validate() ?? false) {
      ...
      final GenericResponseModel<NetworkManagerMixin> response = await networkManager.getUyeBilgileri(emailController.text, password: encodedPassword, getFromCache: false);
      dialogManager.hideAlertDialog;
      if (response.success == true) {
        for (AccountResponseModel item in response.data) {
          if (!CacheManager.accountsBox.containsKey(item.email)) {
            await CacheManager.setHesapBilgileri(AccountModel.instance);
            await CacheManager.setAccounts(response.data!.first..parola = encodedPassword);
            Get.back(result: true);
            await Get.offAndToNamed("/addCompany");
        ...
        await dialogManager.showAlertDialog(response.message ?? "");
      }
    }
  }
      ...
      if (response.success == true) {
        ...
        for (AccountResponseModel item in response.data!) {
          if (!CacheManager.accountsBox.containsKey(item.email)) {
            ...
            await Get.offAndToNamed("/addCompany");
            await CacheManager.setHesapBilgileri(AccountModel.instance);
            await CacheManager.setAccounts(item..parola = encodedPassword);
            dialogManager.showSuccessSnackBar("Başarılı");
          } else {
